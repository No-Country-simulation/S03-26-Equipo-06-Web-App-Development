...
export default function InfoCardDashboard({ data }: { data: Testimonio }) {
  ...
  return (
    <>
      <button
        onClick={() => router.back()}
        className='absolute top-18 right-6 cursor-pointer rounded-md px-2 py-3 text-sm text-gray-600 sm:top-24 lg:right-10'
      >
        <Undo2 />
      </button>
        ...
        <div className='mx-auto w-full rounded-2xl border border-gray-200 bg-white p-4 shadow-md sm:p-6 md:max-w-5xl'>
          ...
          <div className='mb-6 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between'>
            <div>
              ...
              <p className='mt-1 pt-2 text-sm text-gray-600'>{data.id_usuario}</p>
            </div>
          </div>
